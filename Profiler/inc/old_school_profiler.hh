...
private:
    ...
    int writer_index;
    bool m_Running;
    pthread_t m_ThreeadID;
    pthread_mutex_t m_Mutex;
    bool m_Done;
    std::vector<std::queue>* m_Queue;
...
public:
    ...
    {
        ProfileWriter* writer = static_cast<ProfWriter*>(args);
        writer->execute(writer->m_Queue);
    }

    void start(std::vector<std::queue>* queue)
    {
        m_Queue = queue;
        pthread_create(&m_ThreeadID, NULL, ProfWriter::thread_func, (void*)this)
        m_Running = true;
    }
